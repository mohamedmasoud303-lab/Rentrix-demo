...
// FIX: `settings` is not a direct property of AppContext, it's inside `db`.
    ...
    const { contracts, invoices, tenants } = db;

    const notificationsData = useMemo(() => {
        if (!settings || !contracts || !invoices || !tenants) return { expiring: [], overdue: [], count: 0 };

        const expiringContracts = contracts.filter(c => {
            if (c.status !== 'ACTIVE') return false;
            const endDate = new Date(c.end);
            const alertDate = new Date();
            alertDate.setDate(alertDate.getDate() + settings.contractAlertDays);
        ...
        return {
            ...
            count: expiringContracts.length + overdueInvoices.length
        };
    }, [contracts, invoices, tenants, settings]);
    ...
    const { count, overdue, expiring } = notificationsData;
                                ...
                                {overdue.length > 0 && (
                                    <div>
                                        ...
                                        {overdue.map(inv => {
                                             ...
                                             const tenant = contract ? tenants.find(t => t.id === contract.tenantId) : { name: 'غير معروف'};
                                            return (
                                            <Link to="/invoices" onClick={() => setIsOpen(false)} key={inv.id} className="block px-3 py-2 text-sm hover:bg-slate-50 dark:hover:bg-slate-700">
                                                <div className="flex items-start gap-2">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-slate-500">{formatCurrency(inv.amount - inv.paidAmount, settings.currency)}</p>
                                                    </div>
                                                </div>
                                            </Link>
                                        )})}
