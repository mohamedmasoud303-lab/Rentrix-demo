...
const AttachmentsManager: React.FC<AttachmentsManagerProps> = ({ entityType, entityId }) => {
    ...
    const attachments = useMemo(() => 
        db.attachments.filter(att => att.entityType === entityType && att.entityId === entityId)
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            try {
                ...
                dataService.add('attachments', newAttachment as any);
            } catch (error) {
    ...
    return (
        <div className="pt-4 border-t border-border mt-4">
            ...
            <label className="w-full text-center cursor-pointer bg-background hover:bg-opacity-80 text-foreground font-bold py-2 px-4 rounded-md inline-block">
